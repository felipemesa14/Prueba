    ...
    (function () {
        var g, h = {
            modes: {wysiwyg: 1, source: 1}, canUndo: !1, readOnly: 1, exec: function (a) {
                var f, c = a.config, e = c.baseHref ? '<base href="' + c.baseHref + '"/>' : "";
                if (c.fullPage)f = a.getData().replace(/<head>/, "$&" + e).replace(/[^>]*(?=<\/title>)/, "$& &mdash; " + a.lang.preview.preview); else {
                    var c = "<body ", d = a.document && a.document.getBody();
                ...
                d = 80;
                try {
                    var b = window.screen, e = Math.round(0.8 * b.width), c = Math.round(0.7 * b.height), d = Math.round(0.1 * b.width)
                } catch (h) {
                }
                if (!a.fire("contentPreview", a = {dataValue: f}))return !1;
                b = "";
